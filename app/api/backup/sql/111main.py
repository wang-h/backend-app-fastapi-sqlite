...
@app.get("/drugs/demo", response_model=List[schemas.Drug])
def get_drugs_simple(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    drugs = crud.get_drugs(db, skip=skip, limit=limit)
    print(drugs)
    return drugs


@app.get("/drugs/{drug_id}", response_model=schemas.User)
def get_drugs(drug_id: int, db: Session = Depends(get_db)):
    db_user = crud.get_user(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return db_user
